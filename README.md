...
- Если пользователь пытается создать столбец `id` с типом, отличным от `int`, приложение сообщает о замене и автоматически приводит его к корректному `int`.
- Практически все сообщения и шаблоны ошибок вынесены в `constants`.
- Команда `update` отклоняет попытки изменить столбец `ID`, чтобы гарантировать уникальность первичного ключа.
- Устранена ошибка poetry, скрипты перенес в `[project.scripts]` (PEP 621)
```sh
 > poetry check
